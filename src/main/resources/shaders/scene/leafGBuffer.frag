...
void main() {
    ...
    //    float diff;
    //    vec3 norm, halflife;
    //    vec4 vertexCol, vertexColTransl;
    //
    //    // If facing front (normal towards camera)
    ...
    //        halflife = normalize(TBN * texture(leaf_TSHLM_front_t, textureCoord).rgb);
    //        vertexCol = texture(leaf_front, textureCoord);
    //        vertexColTransl = texture(leaf_transl_front, textureCoord);
    //    } else {
    //        vec3 mapNormal = (texture(leaf_TSNM_back, textureCoord).rgb * 2.0 - 1.0);
    //        norm = normalize(TBN * mapNormal);// TBN maps from tangent space to world space
    //        diff = max(dot(norm, -lightDir), 0.0f);
    //        halflife = normalize(TBN * texture(leaf_TSHLM_back_t, textureCoord).rgb);
    //        vertexCol = texture(leaf_back, textureCoord);
    //        vertexColTransl = texture(leaf_transl_back, textureCoord);
    //    }
    //    if (vertexCol.a < 0.01) {
    //        discard;
    //    }
    //
    //    const float SQRT6 = 0.40824829046386301636621401245098;
    //    const float SQRT2 = 0.70710678118654752440084436210485;
    //    const float SQRT3 = 0.57735026918962576450914878050196;
    //    const float SQRT23 = 0.81649658092772603273242802490196;
    //
    //    float diffTransl =  dot(lightDir, TBN * vec3(-SQRT6, -SQRT2, -SQRT3)) * halflife.x +
    //                        dot(lightDir, TBN * vec3(-SQRT6, SQRT2, -SQRT3)) * halflife.y +
    //                        dot(lightDir, TBN * vec3(SQRT23, 0, -SQRT3)) * halflife.z;
    vec3 mapHalflife;
    vec3 norm;
    vec4 vertexCol;
    vec4 translColour;
